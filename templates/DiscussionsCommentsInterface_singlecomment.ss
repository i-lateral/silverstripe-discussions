...
<% if $Author.Avatar %>
    <img class="avatar" src="{$Author.Avatar.SetWidth(75).URL}" alt="$Author.Nickname" title="$Author.Nickname" />
<% else_if $Gravatar %>
    <img class="avatar" src="$Gravatar.ATT" alt="Gravatar for $Name.ATT" title="Gravatar for $Name.ATT" />
<% end_if %>

<div class="comment-text<% if $Gravatar %> hasGravatar<% end_if %>" id="<% if $isPreview %>comment-preview<% else %>$Permalink<% end_if %>">
	<p>$EscapedComment.Parse(DiscussionContentParser)</p>
</div>
